...
Многопользовательский чат. Пользователь может отправлять сообщения в общий чат и менять своё изображение.

## Использование

1. Во вложенной директории сервера (/server) через консоль установить необходимые модули
   _npm install_
2. Запустить сервер для начала работы
   _node index.js_
3. Из папки course-javascript запустите `npm start`
   В браузере откройте http://localhost:8080/mega-chat
